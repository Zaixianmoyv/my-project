...
const submitForm = (item) => {
  ...
  ruleFormRef.value.validate((valid) => {
    if (valid) {
      axios
        .post("http://localhost:8086/book/manegers/login", form.value)
        .then((reqs) => {
          console.log(reqs);
          ...
          if (reqs.data.code === 0) {
            ...
            sessionStorage.setItem("userid", reqs.data.data.id);
            sessionStorage.setItem("userimage", reqs.data.data.image);
